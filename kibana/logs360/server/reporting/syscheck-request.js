/*
 * Logs360 app - Specific methods to fetch Wazuh syscheck data from Elasticsearch
...
export class SyscheckRequest {
   ...
   */
  async top3agents(gte, lte, filters, pattern = 'logs360-alerts-1.x-*') {
    try {
      const base = {};
   ...
   * @returns {Array<String>}
   */
  async top3Rules(gte, lte, filters, pattern = 'logs360-alerts-1.x-*') {
    try {
      const base = {};
  ...
  async lastTenDeletedFiles(gte, lte, filters, pattern = 'logs360-alerts-1.x-*') {
    try {
      const base = {};
