  ...
  return /*@__PURE__*/(function (ExtendedParser) {
    ...
    anonymous.prototype.parseClassElement = function parseClassElement (_constructorAllowsSuper) {
      ...
      if (this.type === this.privateNameToken) {
        ...
        if (this.type !== tt.parenL) {
          if (node.key.name === "prototype") {
            this.raise(node.key.start, "As classes podem não ter uma propriedade estática privada denominada prototype")
          }
          maybeParseFieldValue.call(this, node)
          this.finishNode(node, "FieldDefinition")
          this.semicolon()
        ...
        if ((node.key.name || node.key.value) === "prototype" && !node.computed) {
          this.raise(node.key.start, "As classes podem não ter uma propriedade estática denominada prototype")
        }
      }
      ...
      if (!node.kind && (node.key.name || node.key.value) === "constructor" && !node.computed) {
        this.raise(node.key.start, "As classes podem não ter um campo estático chamado construtor")
      }
      if (node.kind === "get" && node.value.params.length !== 0) {
        this.raiseRecoverable(node.value.start, "getter não deve ter parâmetros")
      }
      if (node.kind === "set" && node.value.params.length !== 1) {
        this.raiseRecoverable(node.value.start, "setter deve ter exatamente um parâmetro")
      }
      if (node.kind === "set" && node.value.params[0].type === "RestElement") {
        this.raiseRecoverable(node.value.params[0].start, "O setter não pode usar parâmetros de descanso")
      }
    ...
    anonymous.prototype.parseIdent = function parseIdent (liberal, isBinding) {
      ...
      if (this._inStaticFieldValue && ident.name == "arguments") { this.raise(ident.start, "Um inicializador de campo de classe estático pode não conter argumentos") }
      return ident
    };
